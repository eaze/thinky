 ...
 */
function Query(options, query, postValidation, error) {
  var self = this;
  var model = options ? options.model : undefined;
...
/**
 * Builds options block for chaining new Query's
 */
Query.prototype._buildOptions = function(model) {
  var options = {
...
/**
 * Get or set a value in the context of the query. Contexts are passed down to 
 * child queries and models generated for reference in consuming code.
 * @param {String} key The name of the value
 * @param {Value=} value The value to assign `key` to
 * @return {} return the value of `key` if `value` is not passed. Otherise, 
 * assigns the value and returns self for chaining
 */
Query.prototype.context = function(key, value) {
  ...
  if (arguments.length === 1) {
    return this._options.context[key];
  }
  this._options.context[key] = value;
...
Query.prototype._executeCallback = function(fullOptions, parse, groupFormat) {
  ...
  return self._query.run(fullOptions).then(function(result) {
    ...
    if (result != null && typeof result.getType === 'function') {
      ...
      if (resultType === 'AtomFeed') {
        return result.next().then(function(initial) {
          ...
          return self._model._parse(value, { context: self._options.context }).then(function(doc) {
            doc._setFeed(result);
            return doc;
          });
        });
    ...
    if (parse === true) {
      return self._model._parse(result, { context: self._options.context });
    }
  ...
  for(var i=0; i<result.changes.length; i++) {
    (function(i) {
      if (result.changes[i].new_val !== null) {
        promises.push(self._model._parse(result.changes[i].new_val, { context: self._options.context }));
      }
    })(i)
  }
  ...
  util.loopKeys(joins, function(joins, key) {
    if (util.recurse(key, joins, modelToGet, getAll, gotModel)) {
      if (joins[key].type === 'hasOne') {
        self._query = self._query.merge(function(doc) {
          return r.branch(
            ...
            r.table(joins[key].model.getTableName()).getAll(doc(joins[key].leftKey), {index: joins[key].rightKey}).coerceTo("ARRAY").do(function(result) {
              innerQuery = new Query(self._buildOptions(joins[key].model), result.nth(0));
        ...
        self._query = self._query.merge(function(doc) {
          innerQuery = new Query(self._buildOptions(joins[key].model),
        ...
        self._query = self._query.merge(function(doc) {
          if ((model.getTableName() === joins[key].model.getTableName()) && (joins[key].leftKey === joins[key].rightKey)) {
            ...
            return r.branch(
              doc.hasFields(joins[key].leftKey),
              r.object(key, new Query(self._buildOptions(joins[key].model), innerQuery).getJoin(modelToGet[key], getAll, gotModel)._query),
              {}
            )
          }
          else {
            ...
            return r.branch(
              doc.hasFields(joins[key].leftKey),
              r.object(key,
                new Query(self._buildOptions(joins[key].model), innerQuery).getJoin(modelToGet[key], getAll, gotModel)._query),
              {}
            )
          }
        });
      ...
      if (key === 'get') {
        ...
        (function(key) {
          Query.prototype[key] = function() {
            return new Query(this._buildOptions(), this._query[key].apply(this._query, arguments)).default(this._r.error(new Errors.DocumentNotFound().message));
          }
        })(key);
      }
        ...
        (function(key) {
          Query.prototype[key] = function(value, options) {
            ...
            if (error === null) {
              return new Query(this._buildOptions(), this._query[key].call(this._query, value, options), true);
            }
            else {
              return new Query(this._buildOptions(), this._query[key].call(this._query, value, options), true, error);
            }
          }
        })(key);
      }
        ...
        (function(key) {
          Query.prototype[key] = function() {
            ...
            return new Query(this._buildOptions(), this._query[key].apply(this._query, arguments));
          }
        })(key);
      }
