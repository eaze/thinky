...
Model.prototype._parse = function(data, options) {
  var self = this;
  var promises = [];
  var promise;
    ...
    if (Array.isArray(data)) {
      util.tryCatch(function() {
        for(var i=0; i<data.length; i++) {
          data[i] = new self(data[i], options);
      ...
      if (util.isPlainObject(data) && (data.$reql_type$ === "GROUPED_DATA")) {
          ...
          for(var i=0; i<data.data.length; i++) {
            ...
            if (Array.isArray(data.data[i][1])) {
              for(var j=0; j<data.data[i][1].length; j++) {
                newDoc = new self(data.data[i][1][j], options);
              ...
              newDoc = new self(data.data[i][1], options);
              newDoc.setSaved(true);
        ...
        if (data === null) { // makeDocument is true, but we got `null`
          ...
          util.tryCatch(function() {
            var newDoc = new self(data, options);
            newDoc.setSaved(true);
